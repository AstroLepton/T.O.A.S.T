...
if "logmass" in df_euclid.columns:
    df_euclid["HOST_LOGMASS"] = pd.to_numeric(df_euclid["logmass"], errors="coerce")
else:
    df_euclid["HOST_LOGMASS"] = pd.to_numeric(df_euclid["HOST_LOGMASS"], errors="coerce")
...
df = df_euclid[["DES_ID_x", "HOST_LOGMASS"]].merge(
...
df = df.dropna(subset=["zHD", "mB", "x1", "c", "HOST_LOGMASS", "MUERR"])
...
# PRINT SUMMARY
print("=" * 60)
print("Euclid: Hubble Residual vs Host Mass (Mass Step)")
print("=" * 60)
print(f"HOST_LOGMASS range (Euclid): {df['HOST_LOGMASS'].min():.2f} -- {df['HOST_LOGMASS'].max():.2f}")
...
ax.scatter(df["HOST_LOGMASS"], df["hubble_residual"], alpha=0.2, color="gray", s=15, label="Euclid SNe Ia")
...
ax.set_title(f"Euclid: Hubble Residual vs Host Mass (Mass step: {mass_step:.3f} mag)")
ax.set_xlabel(r"$\log_{10}(M_{host}/M_\odot)$ (Euclid)")
...
plt.savefig(out_dir / "euclid_hubble_residual_vs_mass_step.png", dpi=150)
plt.show()
